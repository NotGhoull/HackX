...
export async function main(ns) {
    ...
    if (ns.fileExists("/hackx/hackx.js")) {
        ns.tprint("WARN: HackX already installed, are you sure you want to overwrite it?");
        const overwrite = ns.prompt("Are you sure you want to overwrite the current HackX installation?", {
            type: "boolean",
            choices: ["Yes", "No"],
        });
        await overwrite;
    ...
    const createAlias = await ns.prompt("Create Hax alias?\nThis is recommeneded", {
        type: "boolean",
        choices: ["Yes", "No"],
    });
    if (createAlias) {
...
function printFinishMessage(ns) {
    ns.ui.clearTerminal();
    ns.tprint(`INFO
...
export async function fetchFilesInfo(url) {
...
async function downloadFile(ns, url, localPath) {
...
export async function downloadDirectory(ns, url, path) {
